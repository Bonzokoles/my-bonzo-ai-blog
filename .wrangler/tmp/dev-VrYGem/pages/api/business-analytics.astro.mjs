...
class BusinessAnalyticsEngine {
  ...
  async generatePriceAnalysisReport(period = "3 months") {
    const priceAnalysis = await this.env.PUMO_DB.prepare(`
            ...
            LIMIT 100
        `).all();
    const analysisData = priceAnalysis.results.map((row) => {
      const volatility = (row.max_price - row.min_price) / row.avg_price * 100;
      const trend = row.current_price > row.avg_price ? "increasing" : row.current_price < row.avg_price ? "decreasing" : "stable";
      return {
  ...
  async generateStockAnalysisReport(period = "3 months") {
    const stockAnalysis = await this.env.PUMO_DB.prepare(`
            ...
            LIMIT 100
        `).all();
    const analysisData = stockAnalysis.results.map((row) => {
      ...
      return {
        ...
        restock_frequency: Math.round((row.stock_changes || 0) / 90 * 30),
        // per month
    ...
    return {
      ...
      data: {
        products_analyzed: analysisData.length,
        critical_stock_issues: analysisData.filter(
          (p) => p.recommended_action === "urgent_restock" || p.recommended_action === "low_stock_alert"
        ),
      ...
      summary: {
        ...
        trend: outOfStockProducts > analysisData.length * 0.1 ? "down" : outOfStockProducts < analysisData.length * 0.05 ? "up" : "stable"
      }
    };
  }
  async generateBusinessOverviewReport() {
