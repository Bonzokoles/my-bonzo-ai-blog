...
class BusinessIntelligenceCollector {
  ...
  async collectFullBusinessData(options) {
  ...
  async collectProductsData() {
    const stats = { total_items: 0, processed: 0, created: 0, updated: 0, failed: 0, duration_ms: 0 };
    try {
  ...
  async collectPriceHistory() {
    ...
    for (const product of products.results) {
      try {
        ...
        const discountPercentage = product.price_promo ? (product.price - product.price_promo) / product.price * 100 : null;
        await this.env.PUMO_DB.prepare(`
    ...
    for (const product of products.results) {
      try {
        const previousStock = await this.env.PUMO_DB.prepare(`
                    ...
                    LIMIT 1
                `).bind(product.product_id).first();
        const stockChange = previousStock ? product.stock_quantity - previousStock.stock_quantity : 0;
  ...
  async collectCategoryStructure() {
  ...
  async completeCollection(collectionId, stats) {
    await this.env.PUMO_DB.prepare(`
  ...
  async failCollection(collectionId, error) {
    await this.env.PUMO_DB.prepare(`
            UPDATE data_collection_logs 
