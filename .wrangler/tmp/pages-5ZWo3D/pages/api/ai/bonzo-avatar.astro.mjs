  ...
  return withFeatureMiddleware(
    ...
    async (ctx, requestContext) => {
      try {
        ...
        const heygenKey = env?.HEYGEN_API_KEY || (typeof process !== "undefined" ? process.env.HEYGEN_API_KEY : void 0) || Object.assign(__vite_import_meta_env__, { HOME: process.env.HOME, OPENAI_API_KEY: process.env.OPENAI_API_KEY, OS: process.env.OS, PROMPT: process.env.PROMPT })?.HEYGEN_API_KEY;
        if (!heygenKey) {
          return new Response(
            JSON.stringify({
              success: false,
        ...
        if (action === "speak" && sessionId && text) {
          const accountId = env?.CLOUDFLARE_ACCOUNT_ID || (typeof process !== "undefined" ? process.env.CLOUDFLARE_ACCOUNT_ID : void 0) || Object.assign(__vite_import_meta_env__, { HOME: process.env.HOME, OPENAI_API_KEY: process.env.OPENAI_API_KEY, OS: process.env.OS, PROMPT: process.env.PROMPT })?.CLOUDFLARE_ACCOUNT_ID;
          const openaiKey = env?.OPENAI_API_KEY || (typeof process !== "undefined" ? process.env.OPENAI_API_KEY : void 0) || process.env.OPENAI_API_KEY;
