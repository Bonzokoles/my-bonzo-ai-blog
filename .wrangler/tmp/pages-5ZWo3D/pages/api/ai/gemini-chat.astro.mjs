...
const POST = async ({ request }) => {
  try {
    ...
    const cfAccountId = undefined                             ;
    const aiGatewaySlug = "google_ai_gate";
    if (!cfAccountId) {
      return new Response(
...
Respond in Polish as Bonzo, the door expert.`
    ...
    const reply = data.candidates?.[0]?.content?.parts?.[0]?.text || "Przepraszam, nie mogę odpowiedzieć w tym momencie.";
    return new Response(
