 ...
 * Inteligentny analityk danych sklepu wykorzystujący D1 Analytics Schema
 * 
 * Funkcje:
 * - Analiza wydajności kategorii
 * - Wykrywanie problemów z pokryciem
 * - Sugestie optymalizacji
 * - Raporty UTM i konwersji
 */
...
interface AnalysisRequest {
    query: string;
    type?: 'category' | 'queries' | 'utm' | 'general';
...
// AI Analyst prompts dla różnych typów analiz
...
async function runD1Query(env: any, query: string, params: any[] = []): Promise<any> {
    try {
        if (!env.ANALYTICS_DB) {
...
async function analyzeWithAI(env: any, prompt: string, data: any): Promise<string> {
    try {
...
Wygeneruj krótką analizę (max 300 słów) z konkretnymi rekomendacjami w języku polskim.`;
        ...
        // Używamy Cloudflare AI do analizy
        if (env.AI) {
...
async function getCategoryAnalysis(env: any): Promise<any> {
    const query = `
    SELECT 
      category,
      total_products,
...
async function getLowHitQueries(env: any): Promise<any> {
    const query = `
    SELECT 
      user_query,
      total_requests,
...
async function getUTMPerformance(env: any): Promise<any> {
    const query = `
    SELECT 
      utm_campaign,
...
async function getAISuccessAnalysis(env: any): Promise<any> {
    const query = `
    SELECT 
      date,
      query_type,
...
export const POST: APIRoute = async (context) => {
    return withFeatureMiddleware(
        'ai-rag-chat', // Używamy istniejącego feature ID
        context,
        'public',
        async ({ request, locals }) => {
            try {
                ...
                // Pobierz dane na podstawie typu zapytania
                switch (type) {
                    case 'category':
                    ...
                    case 'general':
                    default:
                        // Pobierz wszystkie dane dla ogólnej analizy
                        data.categories = await getCategoryAnalysis(env);
                        data.lowHitQueries = await getLowHitQueries(env);
                        data.utmPerformance = await getUTMPerformance(env);
    ...
    return withFeatureMiddleware(
        ...
        async ({ url, locals }) => {
            try {
                ...
                // Quick health check i podstawowe statystyki
                const stats = {
